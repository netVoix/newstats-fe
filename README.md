...
* [`src/newstats_fe/styles.cljs`](src/newstats_fe/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/newstats_fe/`](src/newstats_fe/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/newstats_fe/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `newstats-fe` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/newstats_fe/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns newstats-fe.example
  (:require [newstats-fe.config :as config])
